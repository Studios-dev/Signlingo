...
export default function FillIn({ question }: { question: FillInTheBlankQuestion }) {
    ...
    <>
		<p className="text-gray-50 mt-10 max-w-lg text-center">Fill in the blank with the word that fits the best</p>
		...
		<div className="flex gap-4">
			{question.extraWords.filter(v => v != selected).map((v) => (
				<button className="py-2 px-4 border-2 border-white/20 rounded-xl shadow-sm bg-white/10 backdrop-blur-md" onClick={() => setSelected(v)}>{v}</button>
			))}
		</div>
    </>
  );
